...
// Export HR logs (if needed)
export const exportHrLogsApi = async (query: GetHrLogsDto): Promise<Blob> => {
  try {
    const queryParams = new URLSearchParams();
    if (query.hr_id) queryParams.append('hr_id', query.hr_id.toString());
    if (query.action_type) queryParams.append('action_type', query.action_type);
    ...
    const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:3000'}${url}`, {
      method: 'GET',
      headers: {
    ...
    throw new Error('Failed to export HR logs');
  }
};
