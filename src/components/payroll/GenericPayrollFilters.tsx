/**
 * Generic Payroll Filters - Following same structure as GenericAccessLogsFilters
 */

import React, { useState } from 'react';
import { useFilters } from '../../hooks/useFilters';

interface GenericPayrollFiltersProps {
  onFiltersChange: (filters: any) => void;
  onClearFilters: () => void;
  departments?: Array<{ id: number; name: string }>;
}

const GenericPayrollFilters: React.FC<GenericPayrollFiltersProps> = ({
  onFiltersChange,
  onClearFilters,
  departments = []
}) => {
      ...
      {showAdvanced && (
        <div className="px-6 py-4 bg-gray-50">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            {renderSelect('departmentId', 'Department', departments.map(d => ({ value: d.id, label: d.name })))}
            {renderSelect('isPaid', 'Payment Status', paidStatusOptions)}
          </div>
...
export default GenericPayrollFilters;
