...
interface BulkActionsProps {
  ...
  onBulkAssign: (productionIds: string[], assignedTo: string) => void;
  onBulkStatusChange: (productionIds: string[], status: string) => void;
  onBulkPriorityChange: (productionIds: string[], priority: string) => void;
  onBulkDelete: (productionIds: string[]) => void;
...
const BulkActions: React.FC<BulkActionsProps> = ({
  ...
  const handleBulkAssign = () => {
    if (selectedEmployee && selectedProductions.length > 0) {
      onBulkAssign(selectedProductions, selectedEmployee);
      setSelectedEmployee('');
  ...
  const handleBulkStatusChange = () => {
    if (selectedStatus && selectedProductions.length > 0) {
      onBulkStatusChange(selectedProductions, selectedStatus);
      setSelectedStatus('');
      setShowActions(false);
    }
  };
  ...
  const handleBulkPriorityChange = () => {
    if (selectedPriority && selectedProductions.length > 0) {
      onBulkPriorityChange(selectedProductions, selectedPriority);
      setSelectedPriority('');
      setShowActions(false);
    }
  };
  ...
  const handleBulkDelete = () => {
    if (selectedProductions.length > 0) {
      onBulkDelete(selectedProductions);
      setShowDeleteConfirm(false);
      setShowActions(false);
    }
  };

  if (selectedProductions.length === 0) return null;
        ...
        <div className="flex items-center space-x-4">
          <div className="flex items-center">
            ...
            <span className="text-sm font-medium text-green-900">
              {selectedProductions.length} production{selectedProductions.length !== 1 ? 's' : ''} selected
            </span>
          </div>
          <button
              ...
              <div className="flex space-x-2">
                ...
                >
                  ...
                  {employees.map((employee, index) => {
                    const employeeId = (employee.id || index.toString()).toString();
                  ...
                  <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                    ...
                    <div className="mt-2">
                      <p className="text-sm text-gray-500">
                        Are you sure you want to delete {selectedProductions.length} selected production{selectedProductions.length !== 1 ? 's' : ''}? 
