...
import { useSalesUnit, useRemoveTeamFromSalesUnit, useAssignTeamToSalesUnit, useAvailableSalesTeams } from '../../../hooks/queries/useSalesUnitsQueries';
import { useNavbar } from '../../../context/NavbarContext';
import UpdateSalesUnitForm from './UpdateSalesUnitForm';

interface SalesUnitDetailsDrawerProps {
  unit: SalesUnit | null;
  isOpen: boolean;
  onClose: () => void;
  onUnitUpdated?: (updatedUnit: SalesUnit) => void;
  viewMode?: 'full' | 'details-only';  // full = all tabs, details-only = just Details tab
  canUpdate?: boolean; // Whether user can update units
}

const SalesUnitDetailsDrawer: React.FC<SalesUnitDetailsDrawerProps> = ({
  unit,
  isOpen,
  onClose,
  viewMode = 'full',
  ...
  const [activeTab, setActiveTab] = useState<'details' | 'teams' | 'leads' | 'completedLeads' | 'update'>('details');
  ...
  // Fetch detailed data if unit is provided (includes employees, leads, teams, head)
  const { data: unitData, isLoading: isLoadingUnit } = useSalesUnit(
    unit?.id?.toString() || '', 
    { enabled: !!unit?.id }
  );

  // Team management mutations
  const removeTeamMutation = useRemoveTeamFromSalesUnit();
  const addTeamMutation = useAssignTeamToSalesUnit();
  
  // Fetch available teams (unassigned teams) - exactly like production
  const { data: availableTeamsData, isLoading: isLoadingAvailableTeams } = useAvailableSalesTeams(false, {
    enabled: showAddTeamModal && !!unit?.id
  });
  
  // Extract available teams - match production pattern exactly
  ...
  const currentUnit = (unitData && typeof unitData === 'object' && 'data' in unitData) ? (unitData.data as SalesUnit) : unit;
  ...
  // Get all data from unit details API (includes employees, leads, teams, head)
  const teams = (unitData as any)?.data?.teams || [];
  const leads = (unitData as any)?.data?.leads || [];
  const completedLeads = (unitData as any)?.data?.completedLeads || [];
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <div>
                      ...
                      <span className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-purple-100 text-purple-800">
                        {currentUnit.crackedLeadsCount || 0}
                      </span>
                    </div>
                    ...
                    <div>
                      ...
                      <p className="text-lg text-gray-900 font-medium">
                        {typeof currentUnit.conversionRate === 'number' ? `${currentUnit.conversionRate.toFixed(2)}%` : 'N/A'}
                      </p>
                    </div>
                ...
                <div className={`bg-white border border-gray-200 rounded-lg ${isMobile ? 'p-4' : 'p-5'}`}>
                  ...
                  {isLoadingUnit ? (
                          ...
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                              ...
                              <div className="mt-2 flex items-center space-x-4 text-xs text-gray-500">
                                ...
                                <span>Type: {lead.type || 'N/A'}</span>
                                <span>Assigned: {lead.assignedTo?.firstName || 'N/A'} {lead.assignedTo?.lastName || ''}</span>
                              </div>
                    ...
                    <div className="text-center py-8">
                      ...
                      <p className="text-gray-500 text-sm">No active leads found for this unit</p>
                    </div>
                  )}
                </div>
              </div>
                ...
                <div className={`bg-white border border-gray-200 rounded-lg ${isMobile ? 'p-4' : 'p-5'}`}>
                  ...
                  {isLoadingUnit ? (
                    ...
                    <div className="space-y-3">
                      {completedLeads.map((completedLead: any) => (
                        <div key={completedLead.id} className="p-4 bg-emerald-50 rounded-lg border border-emerald-200">
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                              <h4 className="text-sm font-medium text-gray-900">{completedLead.lead?.name || 'Unknown Lead'}</h4>
                              <p className="text-sm text-gray-600 mt-1">
                                {completedLead.lead?.email} • {completedLead.lead?.phone}
                              </p>
                              <div className="mt-2 flex items-center space-x-4 text-xs text-gray-500">
                                <span>Amount: ${completedLead.amount?.toLocaleString() || '0'}</span>
                                <span>Employee: {completedLead.employee?.firstName || 'N/A'} {completedLead.employee?.lastName || ''}</span>
                              </div>
                              <div className="mt-1 text-xs text-gray-500">
                    ...
                    <div className="text-center py-8">
                      ...
                      <p className="text-gray-500 text-sm">No completed leads found for this unit</p>
                    </div>
                  )}
                </div>
              </div>
...
export default SalesUnitDetailsDrawer;
